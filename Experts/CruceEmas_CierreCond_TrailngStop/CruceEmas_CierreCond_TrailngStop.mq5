...
input int    periodo_ema_lenta  = 25;
...
//========================= LOOK & FEEL (solo fuera de optimización)
...
void SetupChartBeauty()
{
   ...
   // Título + subtítulo
   MakeLabel("HUD_Title", "🤖 CruceEMAs TS (ATR)", x, y, clrGold, 12, font_title);
   y += line_h + gap_title;
...
double InitialSLDistance() // ATR y stops level (simple y robusto)
{
   double d_atr  = (ArraySize(ATR)>1 ? SL_ATR_Mult * ATR[1] : 0.0);
   double d_slvl = (StopsLevelPoints()+1) * _Point; // +1 punto de margen
...
void trailing_stop_per_ticket(ulong ticket)
{
   ...
   {
      if(bid <= open + step) return;                         // arma solo con avance >= step
...
bool is_above_ema_lenta() { return EMA_RAPIDA[1] > EMA_LENTA[1]; }
bool is_below_ema_lenta() { return EMA_RAPIDA[1] < EMA_LENTA[1]; }

// ¡OJO! Mantengo tus funciones tal cual (nomenclatura invertida en origen)
bool is_going_down()
{
...
bool cruce_compra()
{
   double pend_lenta  = EMA_LENTA[1]  - EMA_LENTA[2];
   double pend_rapida = EMA_RAPIDA[1] - EMA_RAPIDA[2];
   return (EMA_RAPIDA[2] < EMA_LENTA[2]) &&
          (EMA_RAPIDA[1] > EMA_LENTA[1]) &&
          (pend_rapida > 0) && (pend_lenta > 0);
}
...
int OnInit()
{
   ...
   if(EMA_RAPIDA_handle==INVALID_HANDLE || EMA_LENTA_handle==INVALID_HANDLE || ATR_handle==INVALID_HANDLE)
   {
      Print("Error creando indicadores. R:",EMA_RAPIDA_handle," L:",EMA_LENTA_handle," ATR:",ATR_handle);
      return INIT_FAILED;
   }
...
void OnTick()
{
   ...
   // HUD sencillo (solo en modo normal)
   if(!gOptimizing)
   {
      double pendR = (ArraySize(EMA_RAPIDA)>2? EMA_RAPIDA[1]-EMA_RAPIDA[2] : 0.0);
      double pendL = (ArraySize(EMA_LENTA )>2? EMA_LENTA [1]-EMA_LENTA [2] : 0.0);
      string hud =
         ...
         "Pend R: " + DoubleToString(pendR,_Digits) + " | Pend L: " + DoubleToString(pendL,_Digits);
      Comment(hud);
   }
   ...
   if(no_posicion && time_flag && ArraySize(ATR)>1 && ArraySize(EMA_RAPIDA)>2 && ArraySize(EMA_LENTA)>2)
   {
      bool long_sig  = cruce_compra() && is_above_ema_lenta() && is_going_up();
      bool short_sig = cruce_venta()  && is_below_ema_lenta() && is_going_down();
   ...
   // No hay bloque de "cierre por señal contraria".
   // Cierre solo por SL inicial o trailing stop.
}
